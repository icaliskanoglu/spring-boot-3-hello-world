...
COPY target/spring-boot-3-hello-world-1.0.0-SNAPSHOT.jar /app/app.jar
COPY run.sh /app/run.sh

# 2. If the Grafana Agent or OpenTelemetry Collector is running on a different host, set the URL:
ENV OTEL_EXPORTER_OTLP_ENDPOINT=grafana-k8s-monitoring-grafana-agent.aws-prometheus.svc.cluster.local:4318
...
ENV OTEL_SERVICE_NAME=petclinic-poc
...
ENV OTEL_RESOURCE_ATTRIBUTES=deployment.environment=sandbox,service.version=1.0
